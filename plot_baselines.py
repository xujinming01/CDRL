...
def load_results(root_dir_or_dirs, enable_progress=True, enable_monitor=True,
                 verbose=False):
    """
    ...
    for rootdir in rootdirs:
        ...
        for dirname, dirs, files in os.walk(rootdir):
            ...
            if ({'metadata.json', 'monitor.json', 'progress.json',
                 'progress.csv'}.intersection(files) or
                    any([f for f in files if monitor_re.match(
                        f)])):  # also match monitor files like 0.1.monitor.csv
                ...
                if enable_progress:
                    if osp.exists(progcsv):
                        try:
                            ...
                            print('skipping progress file in ',
                                  dirname, 'empty data')
                    elif verbose:
...
def symmetric_ema(xolds, yolds, low=None, high=None, n=512, decay_steps=1.,
    ...
    """
    xs, ys1, count_ys1 = one_sided_ema(xolds, yolds, low, high, n, decay_steps,
                                       low_counts_threshold=0)
    _, ys2, count_ys2 = one_sided_ema(-xolds[::-1], yolds[::-1], -high, -low, n,
                                      decay_steps, low_counts_threshold=0)
    ys2 = ys2[::-1]
...
def plot_results(
    ...
    figsize = figsize or (6 * ncols, 4.8 * nrows)
    ...
    for (isplit, sk) in enumerate(sorted(sk2r.keys())):
        ...
        for result in sresults:
            ...
            if average_group:
                ...
                if resample:
                    x, y, counts = symmetric_ema(x, y, x[0], x[-1], resample,
        ...
        if average_group:
            for group in groups:
                ...
                g2l[group] = l
                if shaded_err:
                    ax.fill_between(usex, ymean - ystderr, ymean + ystderr,
                                    color=color, alpha=.4)
                if shaded_std:
                    ax.fill_between(usex, ymean - ystd, ymean + ystd,
                                    color=color, alpha=.2)
        ...
        if any(g2l.keys()):
            ax.legend(
                g2l.values(),
                ['%s (%i)' % (g, g2c[g]) for g in
                 g2l] if average_group else g2l.keys(),
